...
from database import async_engine, sync_engine, Base
...
def get_123_sync():
    with sync_engine.connect() as conn:
        ...
        print(f"{res.first()=}")  # Кортеж с одной строкой

        res = conn.execute(text("SELECT 1,2,3 union select 4,5,6"))
        print(f"{res.all()=}")  # Кортеж со строками
...
class SyncCore:
    @staticmethod
    def create_tables():
        # Отключить логирование SQLAlchemy
        sync_engine.echo = False
        # Удаление всех таблиц
        Base.metadata.drop_all(sync_engine)
        # Создать все таблицы, которые хранятся в метаданных
    ...
    def insert_workers():
        with sync_engine.connect() as conn:
            ...
            #     ('AO Bobr'),
            #     ('OOO Volk');"""
            # Запрос составленный через query builder
            ...
            conn.execute(stmt)  # text() используется, если вводится сырой SQL запрос
            conn.commit()
    ...
    def select_workers():
        with sync_engine.connect() as conn:
            ...
            """
            result.all() - возвращает все столбцы (список кортежей)
            result.scalars().all() - возвращает первый столбец в каждой строки
            result.mappings().all() - возвращает список словарей, которые содержат строки 
            """
    ...
    def update_worker(worker_id: int = 2, new_username: str = "Misha"):
        with sync_engine.connect() as conn:
            """
            Сырой SQL запрос (для замены значений нельзя использовать f строки)
            stmt = text("UPDATE workers SET username=:username WHERE id=:id")
            
            Замена значений в запросе через bindparams защищает от SQL инъекций
            ...
            stmt = (
                ...
                .filter_by(id=worker_id)  # Можно использовать вместо where
            )
            conn.execute(stmt)
            conn.commit()
            # query = update(workers_table).filter_by(id=worker_id)
